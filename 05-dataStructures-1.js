...
for (let [key1, infoLine] of flightsArr.entries()) {
    let infoLineArr = infoLine.split(';');
    for (let [key2, word] of infoLineArr.entries()) {
        if (key2 === 0) {
            word = word.replace(/_/g, ' ');
            if (word.includes('Delayed'))
                word = `🔴${word}`;
        }
        if (key2 === 1 || key2 === 2) {
            word = `${key2 === 1 ? 'from ' : 'to '}${word.slice(0, 3).toUpperCase()}`;
        }
        if (key2 === 3) {
            word = `(${word.replace(':', 'h')})`;
        }
        infoLineArr[key2] = word;
    }
    flightsArr[key1] = `${infoLineArr.join(' ').padStart(45)}\n`;
}
